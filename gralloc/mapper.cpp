...
int gralloc_lock_ycbcr(gralloc_module_t const* /*module*/,
                       ...
                       android_ycbcr *ycbcr) {
//    ALOGE("[Gralloc] entered into lock_ycbcr");
    if (!ycbcr) {
        ALOGE("[Gralloc] gralloc_lock_ycbcr got NULL ycbcr struct");
        return -EINVAL;
    }
    if (private_handle_t::validate(handle) < 0)
    {
        ALOGE("[Gralloc] gralloc_lock_ycbcr got BAD handle");
        return -EINVAL;
    }
    ...
    private_handle_t* hnd = (private_handle_t*)handle;
  //  ALOGE("[Gralloc] gralloc_lock_ycbcr check handle format");
   /* if (hnd->reqFormat != HAL_PIXEL_FORMAT_YCbCr_420_888) {
        ALOGE(" [Gralloc] gralloc_lock_ycbcr can only be used with "
              "HAL_PIXEL_FORMAT_YCbCr_420_888, got %x instead",
              hnd->reqFormat);
        return -EINVAL;
    }*/

    uint8_t *cpu_addr =  (uint8_t *)(hnd->base);
    ...
    size_t vOffset;
    size_t cStep;
    //ALOGE("[Gralloc]   locking memory  with width %d and height %d",
      //    w , h);
    yStride = w;
    cStride =w;
    yOffset = 0;
...
//
//    ALOGE("[Gralloc] gralloc_lock_ycbcr success. usage %x   ycbcr.y: %p, .cb: %p, .cr: %p, "
//       ".ystride: %zu , .cstride: %zu, .chroma_step: %zu", usage,
